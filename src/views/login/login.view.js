 ...
 */
const renderLoginPage = (uid, options = {}) => {
    ...
    if (step === "OTP") {
        fieldsHtml = `
            ...
            <div class="form-group">
                ...
                <input type="text" id="otp" name="otp" placeholder="Enter OTP" required autocomplete="one-time-code" />
            </div>
        `;
        btnText = "Verify OTP";
    } else {
        fieldsHtml += `<input type="hidden" name="step" value="IDENTIFIER" />`;
        ...
        if (isPassword && isOTP) {
            fieldsHtml += `
                <input type="hidden" id="loginMethod" name="loginMethod" value="PASSWORD" />
                <div class="auth-toggle">
                    <button type="button" class="auth-toggle-btn active" id="toggle-password">Password</button>
                    <button type="button" class="auth-toggle-btn" id="toggle-otp">OTP</button>
                </div>
    ...
    let socialHtml = "";
    if (socialLogins && step === "IDENTIFIER") {
        let buttons = [];
...
<body>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <p>${step === "OTP" ? "Enter the OTP sent to your device" : "Sign in to your account"}</p>
        </div>
        ...
        <div class="card-footer">
            Don&rsquo;t have an account? <a href="/oauth2/v1/interaction/${uid}/signup">Sign up</a>
        </div>
    </div>
    ${scripts()}
</body>
